...
def dimension (α : Type u) (γ : Type u) [CommRing γ] := α → γ
...
variable {α} {dim1 : dimension α ℤ} {dim2 : dimension α ℝ}
#check dim1=dim2
namespace dimension
...
def dimensionless (α γ: Type u) [CommRing γ] : dimension α γ := Function.const α 0
instance (α γ: Type u) [CommRing γ] : One (dimension α γ) := ⟨dimension.dimensionless α γ⟩
instance (α γ: Type u) [CommRing γ] : Nonempty (dimension α γ) := One.instNonempty
noncomputable instance (α γ: Type u) [CommRing γ] (a b : dimension α γ ) : Decidable (a = b) :=
...
variable {α γ: Type u} [CommRing γ]
...
protected def pow {γ1 γ2 γ3} [HMul γ2 γ1 γ3] [CommRing γ1] [CommRing γ3]: dimension α γ1 → γ2 → dimension α γ3
| a, n => fun i => n * (a i)
...
instance  {γ1 γ2 γ3} [HMul γ2 γ1 γ3] [CommRing γ1] [CommRing γ3] : HPow (dimension α γ1) γ2 (dimension α γ3) := ⟨dimension.pow⟩
instance : HPow (dimension α γ) γ (dimension α γ) := ⟨dimension.pow⟩
